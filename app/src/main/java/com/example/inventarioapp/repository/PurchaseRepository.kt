...
class PurchaseRepository {
    ...
    fun getActivePurchaseItem(): Flow<List<PurchaseItem>>{
    ...
    suspend fun addPurchasedItem(client: Clients?, item: PurchaseItem): Result<Unit>{
        return try {
            db.runTransaction { transaction ->
                ...
                if (snapshot.exists()){
                    ...
                    transaction.update(currentPurchaseDoc, "total", items.sumOf { it.subtotal })
    ...
    fun deletePurchaseItem(updateItem: PurchaseItem) {
        val currentPurchaseDoc = db.collection(FirestorePaths.Collections.PURCHASES).document(FirestorePaths.Documents.CURRENT_PURCHASE)
        db.runTransaction { transaction ->
            val snapshot = transaction.get(currentPurchaseDoc)
            val purchase = snapshot.toObject(Purchase::class.java) ?: return@runTransaction
